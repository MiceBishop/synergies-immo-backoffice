...
export function LeasesPage() {
  ...
  const unitLabel = (l: LeaseRow) =>
    l.unit
      ? `${l.unit.reference}${l.unit.building ? ` — ${l.unit.building.name}` : ''}`
      : '—'
  ...
  const columns = useMemo<ColumnDef<LeaseRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Locataire',
        enableSorting: false,
        cell: ({ row }) => tenantLabel(row.original),
      },
      {
  ...
  return (
    <div className="space-y-6">
        ...
        toolbar={
          <>
            <DataTableFacetedFilter
              ...
              onChange={(v) =>
                setStatusFilter(v as Enums<'lease_status'>[])
              }
            />
            <DataTableDateRangeFilter
              title="Début entre"
      ...
      <ConfirmDeleteDialog
        ...
        description={
          deleting
            ? `Le bail ${tenantLabel(deleting)} / ${unitLabel(deleting)} sera définitivement supprimé. Les paiements liés bloqueront la suppression si présents.`
            : ''
        }
      />
    </div>
